  ...
  return (
    <div className="px-5 py-4 grid grid-cols-12 gap-5">
      ...
      <div className=" bg-white p-6">
          <h1 className="text-3xl font-thick">Shopping Cart</h1>
          <p className="text-end text-sm text-gray-600">Price</p>
          <hr className="text-gray-light"/>
          {cartItems.length > 0 ? (
            <div>  
            {cartItems.map((item, index) => (
        <div className="flex border-b border-gray-light bg-white items-center gap-4" key={index}>
        <img className="w-60" src={item.image}/>
        ...
        <div className="border-3 border-yellow-light inline px-3 py-1 rounded-2xl space-x-4 font-semibold text-sm">
          {item.quantity === 1 ?
      ...
      ))}</div>) : <p className="text-center mt-2 text-gray-600 font-thin">No items</p> }
     
      <p className="text-end font-light mt-2">Subtotal ({cartItems.length} item): <span className="font-bold text-lg">${total}</span> </p>
      </div>
      ...
      {cartItems && (
        <div className="bg-white p-6  flex flex-col gap-2  items-center">
          <p className="text-end text-lg  mt-2">Subtotal ({cartItems.length} item): <span className="font-bold text-lg">${total}</span> </p>
          <button className="bg-yellow-light font-medium text-xs px-4 py-2.5 rounded-3xl hover:cursor-pointer hover:bg-yellow-400 w-full"> Proceed to checkout</button>
          <p onClick={()=> {
            dispatch(clearItems())
          }} className="text-red-600 text-sm mt-4 hover:cursor-pointer">Clear Cart</p>
        </div>
      )}
      <BestSellersSection/>
    </div>
